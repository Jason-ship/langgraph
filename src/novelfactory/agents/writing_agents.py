...
### 0. 网文爽感铁律（本块最优先，违反整章重写）
- 每章必须包含至少 3 个「可感知爽点」：打脸（反派被当众揭穿/反击）、奖励兑现（系统结算/金手指落地）、升级（能力/地位提升）、装逼（实力展示）、解气（长期压抑的情绪释放）。爽点必须有具体事件支撑，让读者情绪被顶起来
- 开头 300 字内必须出现第一个冲突反制或爽点苗头，前 300 字禁止纯氛围/心理/环境铺垫
- 金手指每章必须实际使用并见效：任务→行动→奖励到账（数值/物品/能力），或反制成功，形成完整闭环
- 反派每章必须"欠打"并至少被主角还手一次（口头反击/数据打脸/规则反制皆可），禁止只憋屈不释放
- 禁止注水：每句话必须推进「剧情/情绪/信息」至少一项；环境与心理描写每段合计不超过 1 句，能用动作和对话替代的绝不描写
- 对话必须制造冲突、推进剧情或揭露信息，禁止寒暄/日常闲聊灌水
- 字数 2500-3000 字，信息密度优先，宁可短不可水
