...
| 能力 | 说明 |
|------|------|
| **全自动创作** | 从"一个程序员在末日中发现代码能修改现实"这样的一句话创意，自动生成完整小说 |
| **超长篇支持** | 默认目标 1000 章 × 3000 字/章 = **300 万字**，可自定义章节数 |
| **逐章质量管控** | 每章写完后经 6 维度评审引擎打分，不达标自动重写/润色，保证质量底线 |
| **跨章一致性** | 角色性格、文风、节奏、伏笔 5 维跨章追踪，不会"写着写着角色变了" |
| **题材自适应** | 内置 24 种网文题材阈值（玄幻/仙侠/都市/系统流/悬疑...），不同题材不同标准 |
| **人工干预** | 支持 interrupt 中断，任意章节可暂停等待人工审核，注入修改指导后继续 |
| **中断恢复** | PostgreSQL 检查点持久化，断电重启从上次位置继续，不丢进度 |
| **多媒体生成** | 每章自动生成插图 + 配音（ThreadPoolExecutor 并行） |
| **飞书同步** | 自动创建飞书文档、上传章节内容、推送进度通知 |
| **实时监控** | SSE 流式推送写作进度、Token 用量、质量评分；里程碑章节自动分析 |

### 创作规模

| 参数 | 默认值 | 可配置 | 说明 |
|------|--------|--------|------|
| 目标章节数 | 1000 章 | ✅ 环境变量 | 一部超长篇网文的长度 |
| 每章字数 | 3000 字 | ✅ 环境变量 | 最低 1500 字，低于则自动重写 |
| 总字数 | **~300 万字** | - | 1000 × 3000 |
| 卷数 | 自动拆分 | - | 每卷 50 章，共 20 卷 |
| 质量评分 | 0-100 分 | - | ≥75 通过，55-74 润色，<55 重写 |
| 重写上限 | 5 次/章 | ✅ 常量 | 防死循环 |
| 润色上限 | 2 次/章 | ✅ 常量 | 防死循环 |
| 内存保留 | 最近 100 章 | ✅ 环境变量 | 超出自动压缩（保留摘要+截断正文） |
| 题材阈值 | 24 种 | ✅ 常量 | 仙侠 88 分通过，无敌流 72 分通过 |

### 技术架构规模

| 维度 | 数量 | 说明 |
|------|------|------|
| 根图节点 | 15+ | Main Supervisor 条件路由 |
| Crew 子图 | 5 个 | Setup / Writing / Media / Sync + NodeSpec 动态注册 |
| Writing Crew 节点 | 10 个 | 含 context_builder / planner / critic / writer / verdict / refiner / corrector |
| 评审维度 | 6 个 | 四维LLM + 程序化 + 跨章 + 辩论 |
| 决策规则 | 9 条 | 含毒点强制重写、双向用尽强制PASS、四维失败降级 |
| 程序化传感器 | 3 个 | AI味8维检测 / 老书虫爽点毒点 / 跨章5维一致性 |
| 数据存储 | 4 种 | PostgreSQL + Milvus + Neo4j + Redis |
| 飞书工具集 | 21 个域 | IM/文档/表格/日历/审批/任务/视频会议/... |
| Docker 服务 | 7 个 | API + PostgreSQL + Redis + Milvus + Neo4j + Nginx + Tools Proxy |
| 源码文件 | 240 个 Python | src/novelfactory/ 下完整模块化实现 |

### 不做什么

- **不替代人类创作** - 系统是辅助工具，不是替代品。人工审核和指导注入仍然是质量的关键
- **不生成短篇/散文** - 系统面向长篇网文（50章+），短篇建议直接用 ChatGPT
- **不做实时交互** - 每章生成需要 LLM 调用（约 30-90 秒），不是即时聊天
- **不保证每章都精品** - 质量门槛保证下限（≥75分通过），但上限取决于 LLM 能力和创意输入质量
