...
验证 v7.9 并行化重构：4 个 LLM 评审维度
（llm_old_reader / llm_ai_style / debate / four_dim）
通过 asyncio.gather 并行执行而非串行。

测试方式：monkeypatch 将 4 个维度替换为 async 桩函数，
...
def _mock_llm() -> SimpleNamespace:
    """桩 LLM 对象（四个维度桩函数不使用真实 LLM）。"""
    return SimpleNamespace(model="mock-llm", invoke=lambda *a, **k: None)


def _patch_four_dimensions(monkeypatch: pytest.MonkeyPatch) -> dict[str, object]:
    """将 evaluate() 依赖的 4 个 LLM 维度替换为桩函数。
...
class TestEvaluateParallel:
    ...
    async def test_evaluate_parallel_calls(self, monkeypatch: pytest.MonkeyPatch) -> None:
        """4 个维度应并行执行：总耗时显著小于串行耗时（0.3s × 4 = 1.2s）。"""
        engine_mod = "novelfactory.evaluation.verdict.engine"
        sleep_duration = 0.3
        active = 0
        max_active = 0
        ...
        # 并行：4 个维度同时活跃（max_active == 4），总耗时 ≈ 0.3s
        # 串行：每时刻最多 1 个活跃，总耗时 ≈ 1.2s
        assert max_active == 4, f"4 个维度未并行（同时活跃峰值={max_active}）"
        assert elapsed < 1.0, (
            f"evaluate 总耗时 {elapsed:.2f}s，疑似串行执行（并行应≈0.3s，串行≥1.2s）"
        )
...
class TestEvaluateResultsMerged:
    ...
    async def test_evaluate_results_merged(self, monkeypatch: pytest.MonkeyPatch) -> None:
        """4 个维度固定返回值应被正确融合进 VerdictResult。"""
        _patch_four_dimensions(monkeypatch)
